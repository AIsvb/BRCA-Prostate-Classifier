...
# Get OpenSlide path from .env
OPENSLIDE_PATH = os.environ.get('OPENSLIDE_PATH')
if OPENSLIDE_PATH:
    OPENSLIDE_PATH = Path(OPENSLIDE_PATH)
    if not OPENSLIDE_PATH.exists():
        logger.warning(f"OPENSLIDE_PATH {OPENSLIDE_PATH} from .env does not exist.")

# Load OpenSlide dynamically
try:
    if hasattr(os, 'add_dll_directory'):  # Python >= 3.8 on Windows
        if OPENSLIDE_PATH is None or not OPENSLIDE_PATH.exists():
            raise FileNotFoundError(
                "OpenSlide path not found. Specify it in the .env file as OPENSLIDE_PATH "
                "or set the OPENSLIDE_PATH environment variable."
            )
        logger.info(f"Using OpenSlide path: {OPENSLIDE_PATH}")
        with os.add_dll_directory(str(OPENSLIDE_PATH)):
            import openslide
    else:
        import openslide
except ImportError as e:
    logger.error(f"Failed to import OpenSlide: {str(e)}")
    raise
